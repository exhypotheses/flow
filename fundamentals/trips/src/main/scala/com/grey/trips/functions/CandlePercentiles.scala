...
class CandlePercentiles(spark: SparkSession) extends CandlePoints {
  ...
  /**
    ...
    * @param partition: The field that determines the partition within which a quantile is calculated
  ...
  def candlePercentiles(records: DataFrame, partition: String, field: String): Array[Candle] = {
    ...
    val tiles: ParArray[Candle] = partitions.par.map{ x =>
      ...
      val quantiles = data.stat.approxQuantile(col = field,
        probabilities = Array(0.1, 0.25, 0.5, 0.75, 0.9), relativeError = 0)
